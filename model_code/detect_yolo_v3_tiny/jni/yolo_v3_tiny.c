#include "yolo_v3_tiny.h"
#include "vnn_yolov3tiny.h"
#include "yolov3_tiny_process.h"
...
det_status_t model_create(const char * data_file_path, dev_type type)
{
	...
	switch (type) {
		case DEV_REVA:
			sprintf(model_path, "%s%s", data_file_path, "/yolov3_tiny_7d.nb");
			break;
		case DEV_REVB:
			sprintf(model_path, "%s%s", data_file_path, "/yolov3_tiny_88.nb");
			break;
		case DEV_MS1:
			sprintf(model_path, "%s%s", data_file_path, "/yolov3_tiny_99.nb");
			break;
		default:
			break;
	}
	g_graph = vnn_CreateYolov3Tiny(model_path, NULL,
...
{
	yolov3_tiny_preprocess(imageData, data);
}
...
det_status_t model_getresult(pDetResult resultData, uint8_t* data)
{
	...
	yolov3_tiny_postprocess(g_graph, resultData);
...
{
	vnn_ReleaseYolov3Tiny(g_graph, TRUE);
	g_graph = NULL;
}
