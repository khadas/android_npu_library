...
det_status_t model_create(const char * data_file_path, dev_type type)
{
	...
	switch (type) {
		case DEV_REVA:
			sprintf(model_path, "%s%s", data_file_path, "/yolov7_tiny_7d.nb");
			break;
		case DEV_REVB:
			sprintf(model_path, "%s%s", data_file_path, "/yolov7_tiny_88.nb");
			break;
		case DEV_MS1:
			sprintf(model_path, "%s%s", data_file_path, "/yolov7_tiny_99.nb");
			break;
		default:
			break;
	}
	g_graph = vnn_CreateYolov7Tiny(model_path, NULL,
...
{
	yolov7_tiny_preprocess(imageData, data);
}
